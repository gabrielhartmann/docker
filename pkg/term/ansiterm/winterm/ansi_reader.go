...
func (ar *ansiReader) Read(p []byte) (int, error) {
	...
	if len(ar.buffer) > 0 {
		logrus.Debugf("Reading previously cached bytes")
		...
		logrus.Debugf("Read from cache p[%d]: % x", copiedLength, p)
	...
	if err != nil {
		...
		logrus.Debug("No input events detected")
		return 0, nil
	}
	...
	if len(keyBytes) > len(p) {
		logrus.Debugf("Received %d keyBytes, only room for %d bytes", len(keyBytes), len(p))
		ar.buffer = keyBytes[len(p):]
		keyBytes = keyBytes[:len(p)]
	} else if len(keyBytes) == 0 {
		logrus.Debug("No key bytes returned from the translater")
		return 0, nil
	}
	...
	logrus.Debugf("Read        p[%d]: % x", copiedLength, p)
	logrus.Debugf("Read keyBytes[%d]: % x", copiedLength, keyBytes)
	return copiedLength, nil
}
...
func readInputEvents(fd uintptr, maxBytes int) ([]INPUT_RECORD, error) {
	...
	logrus.Debugf("[windows] readInputEvents: Reading %v records (buffer size %v, record size %v)", countRecords, maxBytes, recordSize)
	...
	logrus.Debugf("[windows] readInputEvents: Read %v events", nEvents)
